...
export const { render, bind, update, destroy } = channelScreen;
...
export function bind(container) {
  ...
  // Bind neighbor panel as component
  const neighborPanelContainer = container.querySelector('[data-component="neighbors"]');
  if (neighborPanelContainer && activeChannel) {
    const settings = channelSettingsService.getSettings(activeChannel.id);

    if (_neighborsComponent) {
      _neighborsComponent.destroy();
    }

    _neighborsComponent = new NeighborsComponent(neighborPanelContainer, {
      channelId: activeChannel.name,
      threshold: (settings.specificity ?? 50) / 100,
      limit: 10,
      sortBy: 'similarity',
