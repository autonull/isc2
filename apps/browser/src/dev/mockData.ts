 ...
 */
function generateRelations(topic: string): Relation[] {
  const relationTags = [
    'machine learning',
    'ethics',
    'research',
 ...
 */
export function generatePost(channelId: string, channelName: string, overrides?: Partial<Post>): Post {
...
export const scenarios = {
  ...
  starter: (): MockDataset => {
    ...
    const posts = [
      ...
      generatePost(channel.id, channel.name, {
        content: 'Tip: Create channels about topics you\'re passionate about. The more specific, the better the matches!',
        likeCount: 3,
      }),
    ];
    return {
  ...
  active: (): MockDataset => generateMockDataset({
    channelCount: 15,
    postsPerChannel: 8,
    commentsPerPost: 4,
  }),
