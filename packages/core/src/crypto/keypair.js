...
export async function importKeypair(publicKey, privateKey) {
    const [pubKey, privKey] = await Promise.all([
        globalThis.crypto.subtle.importKey('raw', publicKey.buffer, { name: 'Ed25519', namedCurve: 'Ed25519' }, true, ['verify']),
        globalThis.crypto.subtle.importKey('pkcs8', privateKey.buffer, { name: 'Ed25519', namedCurve: 'Ed25519' }, true, ['sign']),
    ]);
    return {
        publicKey: pubKey,
        privateKey: privKey,
 ...
 */
export async function formatKeyFingerprint(publicKey) {
