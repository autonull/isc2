...
export function createSocialGraphExport(
  userId: string,
  following: SocialConnection[],
  options?: { handle?: string; includeFollowers?: boolean; followers?: SocialConnection[] }
): SocialGraphExport {
  return {
    version: GRAPH_EXPORT_VERSION,
    exportedAt: new Date().toISOString(),
    user: { id: userId, handle: options?.handle },
    following,
    followers: options?.includeFollowers ? options.followers : undefined,
    metadata: { format: 'isc-graph-v1-json', totalFollowing: following.length, totalFollowers: options?.followers?.length },
  };
}
...
export function importFromOPML(opml: OPMLDocument, source?: string): SocialGraphExport {
  const following: SocialConnection[] = opml.outlines.map((outline) => ({
    ...
    source,
    metadata: { feedUrl: outline.xmlUrl, profileUrl: outline.htmlUrl },
  }));

  return {
    version: GRAPH_EXPORT_VERSION,
    exportedAt: new Date().toISOString(),
    user: { id: 'imported', handle: opml.ownerName },
    following,
    metadata: { format: 'opml', totalFollowing: following.length },
  };
}
...
export function findDuplicates(
  ...
  const duplicates: Array<{ existing: SocialConnection; incoming: SocialConnection; matchType: string }> = [];
  const existingById = new Map(existing.map((c) => [c.id, c]));
  const existingByHandle = new Map(existing.filter((c) => c.handle).map((c) => [c.handle!.toLowerCase(), c]));
...
export function mergeSocialGraphs(
  ...
  const merged: SocialGraphExport = {
    ...
    metadata: { ...existing.metadata, format: 'isc-graph-v1-json' },
  };

  const existingIds = new Set(existing.following.map((c) => c.id));
  const existingHandles = new Set(existing.following.filter((c) => c.handle).map((c) => c.handle!.toLowerCase()));
...
export function filterConnectionsBySource(graph: SocialGraphExport, source: string): SocialGraphExport {
  const filtered = graph.following.filter((c) => c.source === source);
  return { ...graph, following: filtered, metadata: { ...graph.metadata, totalFollowing: filtered.length } };
}

export function sortConnectionsByDate(graph: SocialGraphExport, order: 'ascending' | 'descending' = 'descending'): SocialGraphExport {
  ...
  return { ...graph, following: sorted };
}

export async function batchImport(
  connections: SocialConnection[],
  importFn: (connection: SocialConnection) => Promise<boolean>,
  options?: { batchSize?: number; delayBetweenBatches?: number; skipDuplicates?: boolean }
): Promise<ImportProgress> {
  const batchSize = options?.batchSize ?? 10;
  const delayBetweenBatches = options?.delayBetweenBatches ?? 100;
  const skipDuplicates = options?.skipDuplicates ?? true;

  const progress: ImportProgress = { total: connections.length, imported: 0, failed: 0, skipped: 0, current: '', status: 'pending', errors: [] };
...
export function validateGraphExport(graph: SocialGraphExport): { valid: boolean; errors: string[]; warnings: string[] } {
  const errors: string[] = [];
