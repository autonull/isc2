...
async function encryptPayload(plaintext: Uint8Array, _supernodePubKey: Uint8Array): Promise<Uint8Array> {
  const key = await crypto.subtle.generateKey({ name: 'AES-GCM', length: 256 }, true, ['encrypt']);
  const iv = crypto.getRandomValues(new Uint8Array(12));
  const encrypted = await crypto.subtle.encrypt({ name: 'AES-GCM', iv, tagLength: 128 }, key, toArrayBuffer(plaintext));
...
export async function createDelegationRequest(options: DelegationRequestOptions): Promise<DelegateRequest> {
...
export async function decryptResponsePayload(encrypted: Uint8Array, _privateKey: CryptoKey): Promise<Uint8Array> {
  ...
  const decrypted = await crypto.subtle.decrypt({ name: 'AES-GCM', iv, tagLength: 128 }, key, toArrayBuffer(ciphertext));
